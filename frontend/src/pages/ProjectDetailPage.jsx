...
const TABS = [
  ...
  { id: 'mail', label: 'Mail Server', icon: Server },
...
export default function ProjectDetailPage() {
  ...
  const fetchPostfixStatus = () => {
    api.get('/api/smtp/postfix')
      .then(r => setPostfixStatus(r.data))
      .catch(() => {})
  }

  const fetchMailAccounts = () => {
    setLoadingMail(true)
    api.get('/api/smtp/accounts')
      .then(r => {
        setMailAccounts(r.data.accounts || [])
        setMailDomains(r.data.domains || [])
        if (r.data.domains && r.data.domains.length > 0) {
          const mainDom = r.data.domains.find(d => !d.includes('localhost') && d.includes('.'))
          if (mainDom) setMailDomain(mainDom)
        }
      })
      .catch(() => {})
  ...
  const createMailAccount = (e) => {
    e.preventDefault()
    if (!mailUsername || !mailPassword) return
    setMailBusy(true)
    setMailMsg('')
    
    api.post('/api/smtp/accounts/create', {
      username: mailUsername,
      password: mailPassword,
      domain: mailDomain
    })
      .then(r => {
        setMailMsg(`✓ ${r.data.message || 'Mailbox account successfully created!'}`)
        setMailUsername('')
        setMailPassword('')
        fetchMailAccounts()
      })
      .catch(err => {
        setMailMsg(`✗ Error: ${err.response?.data?.error || err.message}`)
      })
      .finally(() => setMailBusy(false))
  }

  const handlePostfixAction = (action) => {
    setActionBusy(`postfix-${action}`)
    api.post(`/api/smtp/postfix/${action}`)
      .then(r => {
        fetchPostfixStatus()
      })
      .catch(() => {})
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="glass-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.1rem', fontWeight: 800 }}>Postfix + Dovecot VPS Mail Server</h3>
                  <p style={{ margin: '4px 0 0 0', fontSize: '0.82rem', color: 'var(--color-text-muted)' }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button 
                    ...
                    disabled={actionBusy === 'postfix-restart'}
                    style={{ height: 32, fontSize: '0.8rem' }}
                  >
                    {actionBusy === 'postfix-restart' ? 'Restarting...' : '🔄 Restart Postfix'}
                  </button>
                  <button 
                    className="btn btn-secondary btn-sm" 
                    onClick={() => handlePostfixAction('flush')}
                    disabled={actionBusy === 'postfix-flush'}
                    style={{ height: 32, fontSize: '0.8rem' }}
                  >
                    {actionBusy === 'postfix-flush' ? 'Flushing...' : '⚡ Flush Mail Queue'}
                  </button>
                </div>
              </div>
              ...
              {postfixStatus ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginTop: 12 }}>
                  <div style={{ padding: 16, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--color-border)', borderRadius: 8 }}>
                    <div style={{ fontSize: '0.74rem', color: 'var(--color-text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Service Status</div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 800, color: postfixStatus.status === 'running' ? 'var(--color-success)' : 'var(--color-danger)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ padding: 16, background: 'rgba(255,255,255,0.02)', border: '1px solid var(--color-border)', borderRadius: 8 }}>
                    <div style={{ fontSize: '0.74rem', color: 'var(--color-text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Mail Delivery Queue</div>
                    <div style={{ fontSize: '1.2rem', fontWeight: 800, color: 'var(--color-text)', marginTop: 4 }}>
                      {postfixStatus.queueCount} Pending Messages
                    </div>
                  </div>
                </div>
              ) : (
                <div style={{ color: 'var(--color-text-muted)', fontSize: '0.8rem' }}>Loading Postfix mail server details...</div>
              )}
            </div>

            {/* Create New Mailbox Account Form Card */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
              
              <div className="glass-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 800 }}>Create VPS Mailbox Account</h3>
                  <p style={{ margin: '2px 0 0 0', fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>
                    Provision a new Unix/Dovecot mail user account mapped to Postfix domains.
                  </p>
                </div>

                <form onSubmit={createMailAccount} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ fontSize: '0.76rem', color: 'var(--color-text-muted)', fontWeight: 700 }}>Username (e.g. info, support)</label>
                    <input 
                      type="text" 
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ fontSize: '0.76rem', color: 'var(--color-text-muted)', fontWeight: 700 }}>Associated Domain Configuration</label>
                    <select
                      value={mailDomain}
                    ...
                    >
                      ...
                      {/* Allow custom domain if none present */}
                      {mailDomains.length === 0 && (
                        <option value={project?.name + '.lk'}>{project?.name + '.lk'}</option>
                      )}
                    </select>
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <label style={{ fontSize: '0.76rem', color: 'var(--color-text-muted)', fontWeight: 700 }}>Access Password</label>
                    <input 
                      type="password" 
                      placeholder="••••••••" 
                      value={mailPassword} 
                  ...
                  {mailMsg && (
                    <div style={{ 
                      fontSize: '0.8rem', 
                      color: mailMsg.startsWith('✓') ? 'var(--color-success)' : 'var(--color-danger)', 
                      fontWeight: 600,
                      padding: '8px 12px',
                  ...
                  <button 
                    ...
                    className="btn btn-primary" 
                    disabled={mailBusy}
                    style={{ height: 38, fontSize: '0.82rem', marginTop: 8 }}
                  >
                    {mailBusy ? 'Creating Mailbox...' : '⚡ Create Local VPS Mailbox'}
                  </button>
                </form>
              </div>

              {/* SMTP Client Configuration Settings Card */}
              <div className="glass-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 800 }}>Client SMTP Settings</h3>
                  <p style={{ margin: '2px 0 0 0', fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>
                    Copy these connection settings into your site application configurations or `.env` secrets files.
                  </p>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>SMTP Outgoing Server</span>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>mail.{mailDomain || 'yourdomain.com'}</span>
                  </div>
                  <div style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Secure STARTTLS Port</span>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>587</span>
                  </div>
                  <div style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Local Port (Unencrypted relay)</span>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>25</span>
                  </div>
                  <div style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Secure SSL/TLS Port</span>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>465</span>
                  </div>
                  <div style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: 10, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Encryption Type</span>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, fontFamily: 'var(--font-mono)' }}>STARTTLS / TLS</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)' }}>Authentication Required</span>
                    <span style={{ fontSize: '0.74rem', padding: '2px 8px', background: 'rgba(16,185,129,0.08)', color: 'var(--color-success)', borderRadius: 4, fontWeight: 700 }}>YES</span>
                  </div>
                </div>
              </div>
            ...
            {/* Local VPS Mail accounts List */}
            <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--color-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)', fontWeight: 700 }}>Active Local VPS Mailbox Accounts ({mailAccounts.length})</span>
                <span style={{ fontSize: '0.74rem', color: 'var(--color-text-muted)' }}>All accounts reside in local Unix home directories</span>
              </div>
              
              {loadingMail ? (
                <div style={{ padding: 32, textAlign: 'center', color: 'var(--color-text-muted)' }}>
                  <RefreshCw size={24} className="animate-spin" style={{ opacity: 0.5, marginBottom: 8, display: 'inline-block' }} />
                  <div style={{ fontSize: '0.8rem' }}>Loading VPS mailbox lists...</div>
                </div>
              ) : mailAccounts.length === 0 ? (
                <div style={{ padding: 48, textAlign: 'center', color: 'var(--color-text-muted)' }}>
                  <Server size={32} style={{ opacity: 0.3, marginBottom: 12 }} />
                  <h4 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700, color: 'var(--color-text)' }}>No VPS Mailbox Accounts Provisioned</h4>
                  <p style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', marginTop: 4 }}>
                    Use the creation form above to configure your first local VPS mailbox account.
                  </p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {mailAccounts.map((a, i) => (
                    <div key={a.username} style={{
                      ...
                      gap: 14,
                      padding: '16px 20px',
                      borderBottom: i < mailAccounts.length - 1 ? '1px solid var(--color-border)' : 'none',
                      transition: 'background 0.2s',
                      flexWrap: 'wrap'
                    }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Globe size={15} color="var(--color-success)" />
                      </div>
                      
                      <div style={{ flex: 1, minWidth: 200 }}>
                        <span style={{ fontSize: '0.82rem', fontWeight: 700, color: 'var(--color-text)' }}>{a.username}@{mailDomain || 'yourdomain.com'}</span>
                        <div style={{ fontSize: '0.74rem', color: 'var(--color-text-muted)', marginTop: 2 }}>
                          System UNIX User: <code style={{ fontFamily: 'var(--font-mono)' }}>{a.username}</code> (UID: {a.uid})
                        </div>
                      </div>

                      <div style={{ fontSize: '0.76rem', color: 'var(--color-text-muted)' }}>
                        Home directory: <code style={{ fontFamily: 'var(--font-mono)' }}>{a.home}</code>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Postfix Logs */}
            {postfixStatus && postfixStatus.logs && (
              <div className="glass-card" style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <h3 style={{ margin: 0, fontSize: '0.92rem', fontWeight: 800 }}>Postfix Mail Server Real-Time Activity Logs</h3>
                <div style={{ 
                  ...
                  {postfixStatus.logs.join('\n') || 'No mail logs recorded.'}
                </div>
              </div>
            )}
